def srcVersion = ''
def branchQual = ''
def dockerImageBase = 'nexus.build.svc.cluster.local:5000/workshop/webui'
def dockerImageTag = ''
                ...
                stage('deploy') {
                    container ('helm') {
                        withCredentials([
                            ...
                            sh """
                                kubectl delete secrets registry-webui --kubeconfig='${k8sConfig}' --namespace '${namespace}' || true

                                kubectl create secret docker-registry registry-webui \\
